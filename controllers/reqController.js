...
exports.exportData = async (req, res) => {
  ...
  const records = await Unibase.find({})
    .sort({ _id: -1 })
  ...
  try {
    await workbook.xlsx.writeFile(`requirements-p${page}.xlsx`).then(() => {
...
exports.getHomePage = async (req, res, next) => {
  ...
  const [
    ...
    Interview.find({ interviewStatus: "Interview Completed" })
      .countDocuments()
      .exec(),
    ...
    Interview.aggregate([
      {
        $match: {
          interviewDate: { $eq: dateToday },
          interviewStatus: { $eq: "Interview Confirm" },
        },
      },
...
exports.postCreatePage = async (req, res) => {
  ...
  if (req.body.isDuplicate === "false") {
    ...
    TodaysRecord.forEach((record) => {
      companyMatch = checkDuplicate(
        record.vendorCompany,
        req.body.vendorCompany
      );
      personMatch = checkDuplicate(
        record.vendorPersonName,
        req.body.vendorPersonName
      );
      if (record.primaryTechStack) {
        stackMatch = checkDuplicate(
          record.primaryTechStack,
          req.body.primaryTechStack
        );
      } else {
      ...
      if (
        (companyMatch && personMatch && stackMatch) ||
  ...
  if (duplicateRecords.length) {
    req.flash(
      "error",
      `Duplicate Record Exists With ID - ${duplicateRecords[0]}.`
    );
    ...
    newReq.save(async (err) => {
      if (!err) {
        ...
        error_create_record =
          "Record ID already exists. New ID updated For you, Proceed to Save";
...
exports.getViewRecordPage = (req, res) => {
  ...
  Unibase.findOne({ reqID: reqID }, (err, foundRecord) => {
    ...
    if (err || null) {
      req.flash("error_msg", "Invalid Record ID");
...
exports.postUpdateRecordPage = async (req, res) => {
  ...
  await Interview.updateMany(
    { reqID: reqID },
    { clientName: req.body.clientCompany },
    { new: true }
  );
